...
import (
	...
	shorturl "shorturl-proxy/services/short-url"
	"shorturl-proxy/services/short-url/proto"
...
func (p *proxy) browserRedirection(ctx *gin.Context, isPublic bool) {
	...
	//此处采用302临时重定向，如果采用301多次访问同一个短链url将不经过代理服务器直接进入目标地址，无法统计访问次数
	ctx.Redirect(http.StatusFound, originalUrl)
}

// 反向代理
func (p *proxy) proxy(ctx *gin.Context, isPublic bool) { //to ask?
	shortKey := ctx.Param("short_key")
	originalUrl, err := p.getOriginalUrl(shortKey, isPublic)
	if err != nil {
		p.log.Error(err)
...
func (p *proxy) getOriginalUrl(shortKey string, isPublic bool) (string, error) {
	grpcClientPool := shorturl.GetShortUrlClientPool()
