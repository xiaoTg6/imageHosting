...
int CacheConn::ZsetIncr(string key, string member)
{
    ...
    // ZINCRBY 成功返回新 score，类型一般是 REDIS_REPLY_STRING
    if (reply->type != REDIS_REPLY_STRING)
    {
        printf("ZINCRBY: %s, member: %s unexpected reply type: %d\n",
               key.c_str(), member.c_str(), reply->type);
        retn = -1;
        goto END;
    }
